...
def main(
    ...
    data_range: str = typer.Option("", help="Range of data to generate predictions for. Format: :"),
) -> None:
    ...
    preprocess_args = deepcopy(config["preprocess_args"])
    ...
    if batch_size != 1:
        raise ValueError(
            "For different batch sizes the prediction gives wrong results. "
            "Please set batch_size=1"
        )
    ...
    datapipe = build_single_datapipe(
    ...
    if data_range:
        data_range_min, data_range_max = list(
            map(int, data_range.split(":"))
        )
    else:
        data_range_min, data_range_max = None, None

    # set output files
    log_file, predictions_file = open_files(
        output_folder,
        checkpoint,
        dataset_config,
        data_range,
    ...
    # ==========================================
    # Base SpecTUS + LoRA adapter 로드
    # ==========================================

    base_checkpoint = Path("checkpoints/SpecTUS_pretrained")

    print("Loading base SpecTUS...")

    base_model = SpectusForConditionalGeneration.from_pretrained(
        base_checkpoint
    )
    ...
    decoder_input_token = generation_config.pop(
        "decoder_input_token", ""
    )

    decoder_input_ids = prepare_decoder_input(
        decoder_input_token,
        tokenizer,
        batch_size
    )
