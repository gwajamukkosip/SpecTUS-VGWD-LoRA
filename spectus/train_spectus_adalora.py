...
class AdaLoraSeq2SeqTrainer(transformers.Seq2SeqTrainer):
    def training_step(self, model, inputs):
        loss = super().training_step(model, inputs)

        # gradient accumulation 마지막 micro-step에서만
        # AdaLoRA rank budget을 업데이트
...
def main(config_file: Path = typer.Option(..., dir_okay=False, help="Path to the config file"),
    ...
    adalora_config = AdaLoraConfig(
        init_r=lora_args.get("init_r", 16),
        target_r=lora_args.get("target_r", 8),
        ...
        tinit=lora_args.get("tinit", 200),
        tfinal=lora_args.get("tfinal", 500),
        deltaT=lora_args.get("deltaT", 10),
        total_step=lora_args.get("total_step", 4000),
        beta1=lora_args.get("beta1", 0.85),
        beta2=lora_args.get("beta2", 0.85),
        orth_reg_weight=lora_args.get("orth_reg_weight", 0.5),
    )

    print("AdaLoRA settings:")
    print("  init_r:", lora_args.get("init_r", 16))
    print("  target_r:", lora_args.get("target_r", 8))
    print("  alpha:", lora_args.get("lora_alpha", 32))
    print("  tinit:", lora_args.get("tinit", 200))
    print("  tfinal:", lora_args.get("tfinal", 500))
    print("  deltaT:", lora_args.get("deltaT", 10))
    print("  total_step:", lora_args.get("total_step", 4000))
    ...
    assert train_using_peft + train_fc_only + custom_freeze <= 1, \
        "Only one of train_using_peft, train_fc_only, custom_freeze can be True"
    ...
    print(
        f"Number of trained parameters: "
        f"{tuned_params}/{total_params} "
        f"= {tuned_params/total_params*100:.2f}%"
    )
